...
import { fetchNoticeOrgList } from '@/api/notice.js'
...
export default {
    ...
    methods: {
        ...
        getData() {
            const data = {
                'AdminTypeInfoID': (getStorage('TYPEID') == 0 || getStorage('TYPEID') == 1) ? 0 : getStorage('PTYPEID'),
                'Condition': {
                    'Keyword': this.key,
                    'StartTime': this.StartTime,
                    'EndTime': this.EndTime,
                    'Type': this.type || 0,
                    'ApplicationId': this.appId || 0
                },
        ...
        getList() {
            ...
            fetchNoticeOrgList(data).then(res => {
                this.data = res.Data
        ...
        exportData() {
            ...
            var url = api.upmshost + 'api/Label/exportorganizaionexpireauthority' + conditionStr
            ...
            common.downLoadFile(url, '组织过期权限列表.xlsx')
        },
        rangeChange(arr) {
            if (arr.length) {
