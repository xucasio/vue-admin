<template>
  <div
    ...
    <cs-table
      ...
      :tree-props="{ children: 'AuthorityList', hasChildren: 'hasChildren' }"
...
import { fetchAuthnewList } from '@/api/role.js'
...
export default {
    ...
    methods: {
        ...
        getList() {
            ...
            fetchAuthnewList(data).then(res => {
        ...
        randomId(arr) {
            if (arr.length) {
                const fn = function(arr) {
                    arr.map(item => {
                        item.keyid = item.ID + Math.random().toString().slice(3, 8)
